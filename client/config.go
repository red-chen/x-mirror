...
func LoadConfiguration(opts *Options) (config *Configuration, err error) {
	...
	// try to parse the old .x-mirror format for backwards compatibility
	matched := false
	...
	switch opts.command {
	// start a single tunnel, the default, simple x-mirror behavior
	case "default":
...
func defaultPath() string {
	...
	return path.Join(homeDir, ".x-mirror")
}
